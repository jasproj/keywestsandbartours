#!/usr/bin/env node
/**
 * KWST v5.2 dry-run price extraction.
 *
 * Pulls every tour with `price === null` from tours-data.json (~428),
 ...
 *
 * Output:
 *   - scripts-staging/v52-kwst-dryrun-raw.json (per-tour records)
 *   - scripts-staging/v52-kwst-dryrun.md (human-readable report)
 *
 * Flag: --dry-run-only  (required; refuses to run otherwise)
 *
 * Usage:
 *   node scripts-staging/run-v52-kwst-dryrun.js --dry-run-only
 *   node scripts-staging/run-v52-kwst-dryrun.js --dry-run-only --limit 20  # quick smoke
 */
...
const RAW_FILE = 'scripts-staging/v52-kwst-dryrun-raw.json';
const REPORT_FILE = 'scripts-staging/v52-kwst-dryrun.md';
...
(async () => {
  ...
  console.log(`KWST null-price tours to evaluate: ${targets.length}`);
  ...
  const lines = [];
  lines.push('# KWST v5.2 Dry-Run Report — null-price tour re-extraction');
  lines.push('');
  lines.push(`**Generated:** ${new Date().toISOString()}`);
  lines.push(`**Branch:** \`feat/kwst-v52-price-extraction\``);
  lines.push(`**Mode:** \`--dry-run-only\` (no writes to tours-data.json)`);
  lines.push('');
  lines.push('## 1. Inputs');
  lines.push('');
  lines.push(`- KWST total tours: 1010`);
  ...
  lines.push(`**Net effect if applied --live:** ${promoTotal} tours flip from "Check availability" → "From $X" (${(promoTotal/targets.length*100).toFixed(1)}% of the 428). ${stayTotal} stay hidden.`);
  lines.push('');
