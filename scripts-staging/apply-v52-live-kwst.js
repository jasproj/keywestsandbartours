#!/usr/bin/env node
/**
 * KWST v5.2 live applicator.
 *
 * Reads scripts-staging/v52-kwst-dryrun-raw.json (the validated 428-tour
 ...
 *
 * Backup: writes the pre-mutation file to /tmp/tours-data-kwst.<ISO>.bak
 *         before overwriting.
 */
...
(function main() {
  if (!fs.existsSync(RAW_FILE)) {
    ...
    console.error('  node scripts-staging/run-v52-kwst-dryrun.js --dry-run-only');
    process.exit(2);
  }
  ...
  // Card-flip count using the same logic as KWST app.js:92-96
  ...
  const backupPath = `/tmp/tours-data-kwst.${ts}.bak`;
  fs.writeFileSync(backupPath, dataRaw);
  console.log(`\nBackup: ${backupPath}`);

  const out = data.tours ? { ...data, tours } : tours;
  // Match original file's encoding: escape all non-ASCII chars to \uXXXX
  // so this re-write doesn't generate spurious diff noise on every tour
  // with diacritics, em-dashes, or smart quotes (~all of them).
